...
class SkiplistsTest : public ::testing::Test {
  protected:
    void SetUp() override { Logger::getInstance().setLogLevel(LogLevel::DEBUG); }
...
TEST_F(SkiplistsTest, BuildSkipPointersWithValidData) {
    ...
    EXPECT_NO_THROW({ skiplists_.buildSkipPointers(term, compressed_data); });
...
TEST_F(SkiplistsTest, BuildSkipPointersWithCorruptedData) {
    ...
    EXPECT_THROW({ skiplists_.buildSkipPointers(term, compressed_data); }, std::invalid_argument);
}
